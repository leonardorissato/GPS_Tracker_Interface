...
function updateInterface() {
  ...
  if (locationDBData.length > 0) {
    ...
    let popuptext = datetext.toDateString() + " - " + timetext + " - ";
    if (parseInt(bikePosition.acc) < 0) {
      // if it is negative, position came from wifi
      popuptext += String(-parseInt(bikePosition.acc)) + "m WiFi";
    } else {
      // if it is positive, it came from gps
      popuptext += String(parseInt(bikePosition.acc)) + "m GPS";
    }
